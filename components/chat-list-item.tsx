...
export default function ChatListItem({
  ...
  return (
    <div
      className={`p-4 border-b border-gray-200 flex cursor-pointer hover:bg-gray-50 ${
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center mt-1">
          {chat.lastMessageStatus === "read" && (
            <BsCheckAll className="h-3.5 w-3.5 text-gray-400 mr-1 flex-shrink-0" />
          )}
        ...
        <div className="flex items-center mt-1.5 space-x-1">
          {chat.phone && (
            <div className="flex items-center text-xs text-gray-500">
              ...
              <span>
                {chat.phone}
                {chat.phoneExt && (
                  <span className="ml-1">+{chat.phoneExt}</span>
                )}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center space-x-1 ml-auto">
            ...
            {chat.unreadCount > 0 && (
              <Badge className="bg-green-600 h-5 w-5 p-0 flex items-center justify-center">
                {chat.unreadCount}
              </Badge>
            )}
          </div>
